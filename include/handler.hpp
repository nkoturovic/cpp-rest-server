...
class ApiHandler {
    api_handler_t<Args...> m_api_handler;
public:
    ApiHandler(api_handler_t<Args...> &&func) : m_api_handler(std::move(func)) { }
    ~ApiHandler() = default;

    restinio::request_handling_status_t operator()(restinio::request_handle_t req, Args...args) const {
        try {
            json_t json_req = parse_json_params(req);
            //std::cout << "json_params: " << json_req << std::endl;
            //std::cout << "path_params: " << json_t{args...} << std::endl;
            json_t resp_json = m_api_handler(json_req, args...); //, args...);
