...
inline void register_routes(rs::Router &router, soci::session &db) 
{
    ...
    router.http_get(rs::make_url("/api/users"),
            rs::make_handler([&db](rs::model::Empty&&) -> nlohmann::json {
                return rs::actions::get_models_from_db<rs::model::User>(db, "users");
            })
    );

    router.http_get(rs::make_url("/api/users/", url::non_negative_decimal_number<std::uint64_t>()),
    ...
    router.http_post(rs::make_url("/api/users"), 
