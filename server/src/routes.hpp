...
inline void register_routes(rs::Router &router, soci::session &db) 
{
   ...
   router.api_get(std::make_tuple("/api/users/", epr::non_negative_decimal_number_p<std::uint32_t>()),
      [&db](rs::model::Empty&&, rs::model::AuthToken &&auth_tok, std::uint32_t id) -> nlohmann::json {
          auto vec = rs::actions::get_models_from_db<rs::model::User>(auth_tok,{.owner_field_name = "id"}, db, "users", "*", fmt::format("id = {}", id));
   ...
   router.api_post(std::make_tuple("/api/users"), 
      [&db](rs::model::User &&user, rs::model::AuthToken &&auth_tok) -> nlohmann::json {
           ...
           rs::actions::insert_model_into_db<rs::model::User>(auth_tok,PermissionParams{.owner_field_name = "id"}, db, "users", std::move(user));
